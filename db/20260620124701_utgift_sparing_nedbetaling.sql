-- +goose Up
CREATE TABLE IF NOT EXISTS fond_sparing (
...
CREATE TABLE IF NOT EXISTS konto_sparing (
  id INTEGER PRIMARY KEY,
  sparekonto_id INTEGER NOT NULL,
  utgift_id INTEGER NOT NULL,
  FOREIGN KEY (sparekonto_id) REFERENCES konto (id),
  FOREIGN KEY (utgift_id) REFERENCES utgift (id)
);

CREATE TABLE IF NOT EXISTS lån_nedbetaling (
  id INTEGER PRIMARY KEY,
